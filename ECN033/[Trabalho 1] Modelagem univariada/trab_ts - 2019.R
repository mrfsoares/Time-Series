...
# SARIMA(0,1,2)(2,1,0)
# SARIMA(2,1,1)(2,1,0)
# SARIMA(2,1,0)(2,1,0) # ajustado de acordo com o nível de significância retornado
...
m_previsao = 12
df.test = tail(cred, m_previsao)
df.train = head(cred, length(cred) - length(df.test))

# mod treino
modt = Arima(df.train, order=c(2,1,0), seasonal=list(order=c(4,1,0), period=12, lambda = 0)); modt


main__ <- 'Previsão das concessões de crédito mensal para pessoas físicas no Brasil'
main_ <- '\npara o ano de 2019 a partir de um ARIMA(2,1,0)(4,1,0)[12]'
main = paste(main__, main_)
plot(forecast(object = modt, h=12, level = 0.95),
     main=main, ylab='R$ (milhões)')
# graficamente parece ter funcionado bem
# adicionando acontecimento real
lines(df.test, col = "red")
legend("topleft", c("previsto", "observado"),
       lty = c(1,1),
       col = c("blue", "red"))
grid()
